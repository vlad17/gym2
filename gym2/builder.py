...
def load_cython_ext(mjpro_path):
    """
    Loads the cythonized Cython extension. This is safe to be called from
...
''')
    ...
    return load_dynamic_ext('cythonized', cext_so_path)
...
class MujocoExtensionBuilder():
    ...
    CYTHONIZED_DIR_PATH = abspath(dirname(__file__))
    MUJOCO_DIR_PATH = join(CYTHONIZED_DIR_PATH, 'mujoco')

    def __init__(self, mjpro_path):
        self.mjpro_path = mjpro_path
        self.extension = Extension(
            'gym2.cythonized',
            sources=[join(self.CYTHONIZED_DIR_PATH, "cythonized.pyx")],
            include_dirs=[
                self.CYTHONIZED_DIR_PATH,
                self.MUJOCO_DIR_PATH,
    ...
    def _build_impl(self):
        ...
        build.build_base = join(self.CYTHONIZED_DIR_PATH, 'generated',
    ...
    def get_so_file_path(self):
        dir_path = self.CYTHONIZED_DIR_PATH
        return join(dir_path, "generated", "cythonized_%s.so" % (
...
class WindowsExtensionBuilder(MujocoExtensionBuilder):
    ...
    def __init__(self, mjpro_path):
        ...
        self.extension.sources.append(
            self.MUJOCO_DIR_PATH + "/gl/dummyshim.c")
...
class LinuxCPUExtensionBuilder(LinuxExtensionBuilder):
    ...
    def __init__(self, mjpro_path):
        ...
        self.extension.sources.append(
            join(self.MUJOCO_DIR_PATH, "gl", "osmesashim.c"))
...
class LinuxGPUExtensionBuilder(LinuxExtensionBuilder):
    ...
    def __init__(self, mjpro_path):
        ...
        self.extension.sources.append(
            self.MUJOCO_DIR_PATH + "/gl/eglshim.c")
        self.extension.include_dirs.append(
            self.MUJOCO_DIR_PATH + '/vendor/egl')
...
class ignore_mujoco_warnings:
    ...
    def __enter__(self):
        self.prev_user_warning = cythonized.get_warning_callback()
        cythonized.set_warning_callback(user_warning_ignore_exception)
        return self

    def __exit__(self, type, value, traceback):
        cythonized.set_warning_callback(self.prev_user_warning)
...
cythonized = load_cython_ext(mjpro_path)
...
functions = dict2()
for func_name in dir(cythonized):
    if func_name.startswith("_mj"):
        setattr(functions, func_name[1:], getattr(cythonized, func_name))

functions.mj_activate(key_path)

# Set user-defined callbacks that raise assertion with message
cythonized.set_warning_callback(user_warning_raise_exception)
