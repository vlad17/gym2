...
from gym2.mujoco.utils import remove_empty_lines
from gym2.builder import build_callback_fn
...
cdef class MjSim(object):
    ...
    def render(self, width=None, height=None, *, camera_name=None, depth=False,
        ...
        if mode == 'offscreen':
            ...
            if self._render_context_window is None:
                from gym2.mujoco.mjviewer import MjViewer
    ...
    def save(self, file, format='xml', keep_inertials=False):
        ...
        if format == 'xml':
            ...
            raise ValueError(
                "Unsupported format. Valid ones are 'xml' and 'mjb'")
